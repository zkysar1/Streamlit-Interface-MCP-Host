...
st.title("🔨 SQL Builder Chat")
st.write("""
### Build and optimize SQL queries with AI assistance
**Host:** Oracle SQL Builder (`oraclesqlbuilder`)
""")
...
def send_to_backend_streaming(messages):
    ...
    payload = {"messages": messages, "host": "oraclesqlbuilder"}
    headers = {"Accept": "text/event-stream", "Content-Type": "application/json"}
    
    # Set execution state
    st.session_state.sql_chat_is_executing = True
                    ...
                    try:
                        ...
                        if current_event == 'tool_call_start':
                            ...
                            try:
                                ...
                                st.session_state.sql_chat_stream_id = stream_id
                            ...
                            try:
                                ...
                                st.session_state.sql_chat_is_executing = False
                                st.session_state.sql_chat_stream_id = None
...
if "sql_chat_messages" not in st.session_state:
    ...
    st.session_state.sql_chat_messages.append({"role": "system", "content": systems_opening_instructions_to_assistant })
    st.session_state.sql_chat_messages.append({"role": "assistant", "content": assistants_opening_instructions_to_user })

if "sql_chat_stream_id" not in st.session_state:
    st.session_state.sql_chat_stream_id = None
    
if "sql_chat_is_executing" not in st.session_state:
    st.session_state.sql_chat_is_executing = False
...
with col1:
    if st.button("🛑 Stop", disabled=not st.session_state.sql_chat_is_executing, key="stop_btn"):
        if st.session_state.sql_chat_stream_id:
            # Send interrupt request
            try:
                response = requests.post(
                    f"http://localhost:8080/host/v1/conversations/{st.session_state.sql_chat_stream_id}/interrupt",
...
if prompt := st.chat_input("Paste Context or answers or questions. . ."):
    ...
    with st.chat_message("assistant"):
        ...
        st.session_state.sql_chat_messages.append({"role": "assistant", "content": full_response})
